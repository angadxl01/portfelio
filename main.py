...
async def handle_text_messages(client, message):
    ...
    if state == "waiting_for_broadcast" and user_id == ADMIN_ID:
        ...
        async def run_join_campaign():
            ...
            try:
                sender_client = Client(f"join_sender_{user_id}_{random.randint(1000,9999)}", api_id=API_ID, api_hash=API_HASH, session_string=session_str, in_memory=True)
                await sender_client.start()
                ...
                for idx, target_user in enumerate(users_to_dm):
                    ...
                    try:
                        # Direct Raw MTProto resolve and send message (Same method used by top professional bots)
                        user_obj = await sender_client.get_users(target_user)
                        if user_obj and not user_obj.is_deleted and not user_obj.is_bot:
                            input_user = await sender_client.resolve_peer(user_obj.id)
                            await sender_client.invoke(
                                functions.messages.SendMessage(
                                    peer=input_user,
                                    message=msg,
                                    random_id=sender_client.rnd_id()
                                )
                            )
                            success += 1
                        else:
                        ...
                        try:
                            input_user = await sender_client.resolve_peer(target_user)
                            await sender_client.invoke(
                                functions.messages.SendMessage(
                                    peer=input_user,
                                    message=msg,
                            ...
                            fail += 1
                            print(f"Retry Failed for {target_user}: {inner_e}")
                    except Exception as outer_e:
                        fail += 1
                        print(f"Failed for {target_user}: {outer_e}")
