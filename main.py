...
async def main():
    # Flask server ko background thread mein start karo
    threading.Thread(target=run_flask, daemon=True).start()
    
    # Bot start karo aur event loop maintain rakho
    await app.start()
    print("Bot is running successfully with explicit event loop!")
    await idle()
    await app.stop()
