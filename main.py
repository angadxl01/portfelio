...
user_messages = {}
temp_join_data = {}
temp_campaign_data = {}  # 👈 Fetched chats store karne ke liye
    ...
    if data == "main_menu":
        ...
        try:
            ...
            async for dialog in sender_client.get_dialogs():
                if dialog.chat.type.name == "PRIVATE" and not dialog.chat.is_self and not dialog.chat.is_bot:
...
async def handle_text_messages(client, message):
    ...
    if state == "waiting_for_broadcast" and user_id == ADMIN_ID:
        ...
        await message.reply_text(f"🚀 Campaign started for {len(selected_targets)} users. Sending messages in background...")
            ...
            try:
                ...
                for target in selected_targets:
                    try:
                        ...
                        success_count += 1
                        await asyncio.sleep(2) # FloodWait delay
                    except Exception as e:
                        fail_count += 1
