    ...
    if state == "waiting_for_broadcast" and user_id == ADMIN_ID:
        ...
        try:
            ...
            async for req in sender_client.get_chat_join_requests(chat_obj.id):
                if req.user:
                    # Filter out deleted accounts and bots automatically
        ...
        async def run_join_campaign():
            ...
            try:
                ...
                for idx, target_user in enumerate(users_to_dm):
                    ...
                    try:
                        # Smart Resolution: Resolve user to register peer cache and verify active state
                        user_obj = await sender_client.get_users(target_user)
                        if user_obj and not user_obj.is_deleted and not user_obj.is_bot:
                            await sender_client.send_message(user_obj.id, msg)
                            success += 1
                        else:
                            fail += 1
