...
async def callback_handler(client, callback_query):
    ...
    if data == "main_menu":
        ...
        await callback_query.message.reply_text("🎁 Naya redeem code aur days is format mein bhejein (jaise: `PROMO50 15`):")
        ...
        if not user_chns:
            await callback_query.message.reply_text(
                "⚠️ Aapne koi channel save nahi kiya hai!",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]])
            )
            return
        ...
        buttons.append([InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")])
        await callback_query.message.reply_text(
            "📢 **Your Saved Channels / Join Request Links:**\n\nKisi channel ko delete karne ke liye neeche click karein:",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        ...
        try:
            ...
            if chn_index < len(saved_channels) and saved_channels[chn_index]['user_id'] == user_id:
                ...
                await callback_query.message.edit_text(
                    "⚠️ Invalid channel or already deleted.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]])
                )
        except Exception as e:
            await callback_query.message.edit_text(f"❌ Error: {str(e)}")
...
async def handle_text_messages(client, message):
    ...
    if state == "waiting_for_broadcast" and user_id == ADMIN_ID:
        ...
        try:
            ...
            user_states.pop(user_id, None)
            await message.reply_text(f"✅ Redeem code created successfully!\nCode: `{code}`\nValidity: {days} Days", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]]))
        except Exception:
            await message.reply_text("⚠️ Invalid format! Sahi format use karein: `CODE DAYS` (jaise `VIP50 30`)")
        ...
        if text in valid_redeem_codes:
            ...
            await message.reply_text(
                f"🎉 **Congratulations!**\nVIP Premium successfully activated for `{days}` days!",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]])
            )
        else:
            await message.reply_text(
                "❌ **Invalid or Expired Code!**\nKripya ek sahi redeem code enter karein.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]])
            )
