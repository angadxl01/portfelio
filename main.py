...
async def callback_handler(client, callback_query):
    ...
    if data == "main_menu":
        ...
        user_chns = [(i, c) for i, c in enumerate(saved_channels) if c['user_id'] == user_id]
        if not user_chns:
            await callback_query.message.reply_text("⚠️ Aapne koi channel save nahi kiya hai!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]]))
            return
        buttons = []
        for index, chn in user_chns:
            buttons.append([InlineKeyboardButton(f"🗑️ Delete: {chn['channel']}", callback_data=f"del_chn_{index}")])
        buttons.append([InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")])
        await callback_query.message.reply_text("📢 **Your Saved Channels:**", reply_markup=InlineKeyboardMarkup(buttons))
    elif data.startswith("del_chn_"):
        try:
            chn_index = int(data.split("_")[2])
            if chn_index < len(saved_channels) and saved_channels[chn_index]['user_id'] == user_id:
                saved_channels.pop(chn_index)
                await callback_query.message.edit_text("✅ Channel deleted successfully!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]]))
            else:
                await callback_query.message.edit_text("⚠️ Invalid channel.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]]))
        except Exception as e:
            await callback_query.message.edit_text(f"❌ Error: {str(e)}")
...
async def handle_text_messages(client, message):
    ...
    if state == "waiting_for_broadcast" and user_id == ADMIN_ID:
        ...
        status_msg = await message.reply_text("⏳ Fetching pending join requests for...")
        ...
        try:
            ...
            # Resolve chat
            chat_obj = await sender_client.get_chat(text)
        ...
        if not msg:
            user_states.pop(user_id, None)
            await message.reply_text(
                "❌ **No Message Set**\nYou haven't set a campaign message yet.\nTap **Set Message Now** first, then come back.",
        ...
        try:
            ...
            await status_msg.edit_text("✅ OTP sent! Send OTP code with spaces (e.g., `1 2 3 4 5`):")
        except Exception as e:
        ...
        try:
            ...
            if "SESSION_PASSWORD_NEEDED" in str(e) or "Password" in str(e):
                ...
                await status_msg.edit_text("🔐 Enter your 2FA password:")
            else:
