...
async def callback_handler(client, callback_query):
    ...
    if data == "main_menu":
        ...
        user_states[user_id] = "waiting_for_target_users"
        await callback_query.message.reply_text(
            "🚀 **Start Mass DM Campaign**\n\nAb un users ke usernames ya IDs ki list bhejo jinko messages bhejne hain (ek line mein ek, ya space dekar):"
        )
...
async def handle_text_messages(client, message):
    ...
    elif state == "waiting_for_target_users":
        targets = text.split()
        msg = user_messages.get(user_id)
        user_accs = [a for a in saved_accounts if a['user_id'] == user_id]
        
        user_states.pop(user_id, None)
        await message.reply_text(f"🚀 Campaign started! Total targets: {len(targets)}. Sending messages in background...")
